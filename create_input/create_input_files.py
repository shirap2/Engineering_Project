...
def get_patient_input(name_for_path: str, organ: Organ):
    ...
    pickle_input_address = f"/cs/usr/{USR}/input/{organ}/pkl_files/{name_for_path}_graph_class_data.pkl"
    graph_image_path = f"/cs/usr/{USR}/input/{organ}/graph_images/{name_for_path}_graph_image.png"

    json_input_address = f"/cs/casmip/bennydv/{organ}_pipeline/" \
                         f"lesions_matching/longitudinal_gt/original_corrected/{name_for_path}glong_gt.json"
    partial_scans_address = f"/cs/casmip/bennydv/{organ}_pipeline/" \
