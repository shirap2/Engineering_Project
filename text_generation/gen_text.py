...
def doesnt_appear_or_lone_lesion(ld,last_node,pattern,vol_array,all_patient_dates,cur_component,doesnt_appear_nodes={}):
    ...
    if last_node != next(iter(doesnt_appear_nodes.keys())):
        if len(doesnt_appear_nodes_times)>1:
            ...
            text += find_when_last_appeared_text(next(iter(doesnt_appear_nodes.keys())),edges_to_node_dict,vol_array,all_patient_dates,last_node)
...
def get_text_splitting(last_nodes,vol_change_percentage,longitudinal_volumes_array,edges_list,volume_change_per_edge_dict):
    text =""
    ...
    text+=f"The current lesion burden is {round(total_cur_vol,2)} cc. "
    ...
    if vol_change_percentage>0:
        change = "increased"
    text+=f"The current tumor burden has {change} by {abs(round(vol_change_percentage))}% since the previous scan "
...
def gen_text_single_node(ld,last_node,nodes2cc_class,edge_vol_change_class,edges_list,longitudinal_volumes_array,all_patient_dates,cur_component,volume_change_per_edge_dict,doesnt_appear_nodes={}):
    ...
    if len(doesnt_appear_nodes)>0:
        ...
        text += doesnt_appear_or_lone_lesion(ld,last_node_val,pattern,longitudinal_volumes_array,all_patient_dates,cur_component,doesnt_appear_cc_dict)
        # return text
    change =""
    if vol_change_percentage<0:
        change = "decreased"
    ...
    if pattern == 'complex_p':
        ...
        if edge_class =="splitting":
            ...
            text += get_text_splitting(last_node,vol_change_percentage,longitudinal_volumes_array,edges_list,volume_change_per_edge_dict)
...
def gen_summary_for_cc(ld,cur_component,longitudinal_volumes_array,nodes2cc_class,all_patient_dates,internal_external_names_dict,doesnt_appear_nodes):
    ...
    text += gen_text_single_node(ld,last_nodes,nodes2cc_class,volume_change_per_edge_dict,edges_dict,longitudinal_volumes_array,all_patient_dates,cur_component,volume_change_per_edge_dict,doesnt_appear_nodes)
